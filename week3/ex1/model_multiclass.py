...
class logistic_regression:
	...
	def __init__(self, num_epocs, X_train, X_test, y_train, y_test, learn_rate):
		...
		self.num_outputs = len(self.y_train)
		self.num_train = len(self.X_train)

		print(self.num_features, self.num_outputs, '  8888 ')
	...
	def SGD(self):   
			...
			while  epoch < self.max_epoch:
				...
				for s in range(0, self.num_train): 
					...
					actual  = self.y_train  # self.train_data[i,self.num_features:]  
	...
	def GD(self):   
			...
			while  epoch < self.max_epoch:
				sum_sqer = 0
				for s in range(0, self.num_train): 
					input_instance  =  self.X_train # self.train_data[s,0:self.num_features]  
					actual  = self.X_train[s,:]   
